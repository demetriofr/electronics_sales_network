...
#### Приложение "Звенья сети" ("network_links"):
...
### Описание задания
- [x] Создать веб-приложение с 
  - [x] API-интерфейсом,  
  - [x] админ-панелью.
- [x] Создать базу данных и использовать миграции.
                                                                                                                                     
### Критерии успешного выполнения данного тестового задания:
- [x] Реализована модель сети по продаже электроники:
  - [x] Cеть представляет собой иерархическую структуру из трех уровней (завод, розничная сеть, ИП):
  - [x] Каждое звено сети ссылается только на одного поставщика оборудования (не обязательно предыдущего по иерархии),
  - [x] Уровень иерархии определяется не названием звена, а отношением к остальным элементам сети, т. е. завод всегда 
находится на уровне 0, а если розничная сеть относится напрямую к заводу, минуя остальные звенья, её уровень - 1.
- [x] Каждое звено сети обладает следующими элементами: 
  - [x] название, 
  - [x] контакты (email, страна, город, улица, номер дома), 
  - [x] продукты (название, модель, дата выхода продукта на рынок), 
  - [x] поставщик (предыдущий по иерархии объект сети), 
  - [x] задолженность перед поставщиком в денежном выражении с точностью до копеек, 
  - [x] время создании (заполняется автоматически при создании).
- [x] Сделан вывод в админ-панели созданных объектов.
  - [x] На страницы объекта сети добавлено: 
    - [x] ссылка на "Поставщика",
    - [x] фильтр по названию города, 
    - [x] в admin actions, который очищает задолженность перед поставщиком у выбранных объектов.
- [x] Используется DRF для создания набора представлений CRUD для модели поставщика:
  - [x] Запрещено обновлять через API поле "Задолженность перед поставщиком".
  - [x] Добавлена возможность фильтрации объектов по определенной стране.
- [x] Настроены права доступа к API так, чтобы только активные сотрудники имели доступ к API
